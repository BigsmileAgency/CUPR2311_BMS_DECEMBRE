<?php
	...
	class WeboramaSkinsGSAPProject extends Project {


		public $position = "left";
		public $BG_image = false;
		public $BG_color = "transparent";
		public $BG_width = "auto";
		public $scalableContent = true;
		public $minWidth = 100;
		public $scalableContentWidth = 300;
		public $scalableContentHeight = 800;
		...
		public function __construct(Campaign &$pCampaign, AnimationManager &$pAm, &$pInDexVersion, &$pCSSVersion, $default_css = null, $default_js = null){
			$template = TemplateFactory::get("weborama-skins");
		...
		public function apply(){
			...
			$udata['position'] =  $this -> position;

			$udata['BG_image'] =  $this -> BG_image;
			$udata['BG_color'] =  $this -> BG_color;
			$udata['BG_width'] =  $this -> BG_width;
			$udata['scalableContent'] =  $this -> scalableContent;
			$udata['minWidth'] =  $this -> minWidth;
			$udata['scalableContentWidth'] =  $this -> scalableContentWidth;
			$udata['scalableContentHeight'] =  $this -> scalableContentHeight;

			//$udata['allowEvents'] = $this -> _has_events;

			if(ucfirst($this -> position) == "Left") {
				$this -> _template -> add(new Asset('skinLeft', AppPath::getPath()."/template/predefined/weborama/skins/assets/skinLeft.js", AssetType::JS));
			} else if(ucfirst($this -> position) == "Right") {
				$this -> _template -> add(new Asset('skinRight', AppPath::getPath()."/template/predefined/weborama/skins/assets/skinRight.js", AssetType::JS));
			}
			...
			if(isset($arr["scalable"]))
				$p3 -> replace("scalable-content", $arr["scalable"]);
			else
				$p3 -> replace("scalable-content",'');
