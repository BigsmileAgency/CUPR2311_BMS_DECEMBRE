<?php
	...
	class DCRMOverlayerVideoProject extends Project {

		protected $_poster;
		protected $_video;
		protected $_autoStopTime = 14900;
		protected $_autoStopVideoTime = 14900;
		...
		public function __construct(Campaign &$pCampaign, AnimationManager &$pAm, &$pInDexVersion, &$pCSSVersion, $default_css = null, $default_js = null){
			$template = TemplateFactory::get("dcrm-overlayer");
		...
		public function setAutoStopVideoTime($pTime){
			$this -> _autoStopVideoTime = $pTime;
		}
		...
		public function apply(){
			...
			//$enginegsap = new GSAPEngine($this -> _animation_manager , new GSAPFormatEngine(), NULL, '_main', true);
			...
			$p3 -> replace("autostopvideo-time",$this -> _autoStopVideoTime);
			...
			if(isset($arr["init"]))
				...
				$p3 -> replace("custom-init",'self.launch();');
			if(isset($arr["vars"]))
